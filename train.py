...
def train_pose_refiner_transformer_model():
    ...
    pose_refiner = Pose_Refiner_Transformer().to(args.device)
    pose_refiner.train()
    ...
    for epoch in range(args.train_epochs):
        ...
        for iteration in tqdm(range(len(loader))):
            ...
            batch['initial_image'] = batch['image'].clone()
            ...
            batch['pred_joints'] = pred_joints

            mpjpe_before_refinement, pampjpe_before_refinement = evaluate(
                pred_joints, batch['gt_j3d'])

            refined_joints = pose_refiner(batch)

            refined_joints = move_pelvis(refined_joints)

            mpjpe_after, pampjpe_after = evaluate(
                refined_joints, batch['gt_j3d'])

            joint_loss = loss_function(refined_joints, batch['gt_j3d']/1000)
            ...
            if(args.wandb_log and iteration % 100 == 0):
                ...
                with torch.no_grad():
                    ...
                    refined_joints = pose_refiner(batch, training=False)
