import torch
from scripts import linearized
...
class DifferentiableImageSampler():
    ...
    def warp_image(self, image, homography, out_shape=None):
        assert isinstance(
            image, torch.Tensor), 'cannot process data type: {0}'.format(type(image))
        assert isinstance(homography, torch.Tensor), 'cannot process data type: {0}'.format(
            type(homography))
        ...
        x, y = self.create_flatten_xy(
            x_steps=out_shape[-1], y_steps=out_shape[-2], device=homography.device)
        grid = self.flatten_xy_to_warped_grid(x, y, homography, out_shape)
        # sample warped image
        warped_img = linearized.grid_sample(
